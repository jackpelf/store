...
int person::gen_value(int a_count, int a_depth, bool is_first )
{
	int ace_count= 0, count = 0, depth, res;
	int i;
	if( is_first)
	{
		for( i=0; i<num; i++)
...
//is_21
	...
	{
		if( ( res = this->gen_value(count + 1, depth - 1, false) ) == 21 )
			return res;
		if( ( res = this->gen_value(count + 11, depth - 1, false) ) == 21 )
			return res;
	}
